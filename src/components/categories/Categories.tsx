      ...
      <div className="flex gap-5">
        <div className="w-[40%] bg-gray-100 p-5">
          <div className="flex items-center justify-between pb-5">
            <p className="text-4xl desktop2:text-5xl font-medium">{title}</p>
        ...
        <div className="flex flex-col justify-between">
            <p className="text-xl desktop2:text-3xl desktop2:font-medium">Выбрать по:</p>
            <div className="w-[500px] flex flex-wrap gap-3">
              {["По популярности", "По возрастанию цены", 'По убывании цены', 'Новинки', 'Скидки'].map((el) => (
                <button key={el} className="bg-gray-100 p-3 hover:bg-green-500 hover:text-white transition-colors duration-300 desktop2:text-2xl">{el}</button>
              ))}
            </div>
          </div>
      </div>
